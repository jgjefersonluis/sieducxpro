...
def lista_de_clientes(request):
    ...
    if queryset:
        clientes = Cliente.objects.filter(
            Q(nome__icontains=queryset) | Q(telefone__icontains=queryset))

    paginator = Paginator(clientes, 5)  # Show 25 contacts per page
    page = request.GET.get('page')
    clientes = paginator.get_page(page)

    return render(request, "clientes/lista_de_clientes.html",
                  {'clientes': clientes})
